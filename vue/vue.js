...
(function (global, factory) {
  ...
  */
   ...
   * 将value对象转成JSON字符串 简单的value直接转为字符串
   */
   ...
   * 解析vnode时判断当前元素是否为slot、component
   */
...
*/
   ...
   * 判断两个对象是否相等
   * 1.如果a和b的引用指向同一个对象肯定相等
   * 2. 如果a和b都是对象的话则需要进一步递归遍历各个属性
   * 3. 否则则判断其基本类型是否相等
   */
 ...
 */
   ...
   * let me = {
    ...
    let dataPath = parsePath('options.data')
    let myData = dataPath(me)
    可以获取到data函数
   */
...

  {
    ...
    var classify = function (str) { return str
      ...
      .replace(/[-_]/g, ''); };
    // 警告提示 通过generateComponentTrace方法追溯来源
   ...
   * 依赖收集，对watcher对象进行订阅
   * 
   * watcher 、 Dep、 observer三者之间的关系
   * a.Dep通过 depend方法进行watcher的订阅， 通过notify方法通知watcher进行更新
   * b.Observer 通过 defineReactive方法去new一个Dep, 在get的是时候触发 depend进行依赖收集， 
   * 在set方法触发notify进行更新
   * c.watcher 在getter方法中访问 observer属性触发 observer的get方法
   */
  ...
  // target为 watcher对象， 通过该方法建立watcher与dep的相互连接
  ...
  */
  ...
  // 从另一个层面考虑，假设b为一个对象， 我们令a = b这时a和b会指向同一个引用， 
  // 此后如果我们通过a做一些操作改变属性， b也会跟着变化，而这种变化通常不是我们希望看到的，这也是克隆的原因
  ...
  // 函数式组件 它不会去new一个Vue实例， 这也就意味着这意味它是无状态 (没有响应式数据)，无实例 (没有 this 上下文)
  ...
  */
  ...
  /* 
  关于mount的执行流程 
  $mount => mountComponent => updateComponent =>_render
   => _update => createPatchFunction(patch) => createElm => insert => removeVnodes  
  */
